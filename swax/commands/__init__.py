...
and discover handlers (init_handler, discover_handler), and the plan command
adds the plan_handler re-export so the cli entry point can register all three
handlers through a single import. Consumers import the commands from here, not
from the sub-cells.
"""
...
__all__: list[str] = ["discover_handler", "init_handler", "plan_handler"]
