...
(`init`, `discover`, `plan`) under their consumer-facing aliases
(`init_handler`, `discover_handler`, `plan_handler`). These tests pin the
facade's surface so the cli entry point can rely on it.
"""

import click
from swax.commands import __all__, discover_handler, init_handler, plan_handler
...
def test_facade_all_lists_exactly_the_three_aliases() -> None:
    ...
    assert __all__ == ["discover_handler", "init_handler", "plan_handler"]
