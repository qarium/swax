...
class TestOpenAIAdapterContract:
    ...
    def test_constructor_is_keyword_only(self, mocker):
        ...
        with pytest.raises(TypeError):
            OpenAIAdapter(sdk_client, model="gpt-test")  # type: ignore[misc]

    def test_constructor_requires_model_kwarg(self, mocker):
        sdk_client = mocker.MagicMock(spec=OpenAI)

        with pytest.raises(TypeError):
            OpenAIAdapter(client=sdk_client)  # type: ignore[call-arg]

    def test_constructor_accepts_client_and_model_keywords(self, mocker):
        sdk_client = mocker.MagicMock(spec=OpenAI)

        adapter = OpenAIAdapter(client=sdk_client, model="gpt-test-model")
    ...
    def test_model_is_a_property_returning_injected_model(self, mocker):
        ...
        assert adapter.model == "gpt-test-model"
